...
int save_one_file(char*filename,char*user)
{
    ...
    char user_id[VALUES_ID_SIZE]={0};
    char file_user_list[VALUES_ID_SIZE]={0};
    int ret=0;
    ...
    //根据user得到userid
    ret=rop_hash_get(conn,USER_USERID_HASH,user,user_id);
    if(ret!=0)
    {
        LOG(FCGI,FCGI_PROC,"hget userid error\n");
        goto END;
    }

    //根据userid组装成私有表民
    strcat(file_user_list,FILE_USER_LIST_);
    strcat(file_user_list,user_id);
    //FILE_USER_LIST_17
    //
    ...
    //文件所属用户名
    ret=rop_hash_set(conn,FILEID_USER_HASH,file_id,user);
    if(ret!=0)
    {
    ...
    ret=rop_hincrement_one_field(conn,FILE_REFERENCE_COUNT_HASH,file_id,1);
    if(ret==-1)
    {
        LOG(FCGI,FCGI_PROC"add reference %s error",file_id);
        return -1 ;
    }
...
int select_files(int fromId,int count,char*user,char *cmd,char**out)
{
    int retn=0;
    char*p=NULL;
    char user_id[VALUES_ID_SIZE]={0};
    ...
    retn=rop_hash_get(conn,USER_USERID_HASH,user,user_id);
    if(retn!=0)
    {
        LOG(FCGI,FCGI_PROC,"hget userid error\n");
        goto END;
    ...
    {
        LOG(FCGI,FCGI_PROC,"shareFile is ok");
        strcat(file_user_list,FILE_PUBLIC_LIST);
    }
