...
//    rop_test_reply_type(reply);
    ...
    *get_num = max_count;
    ...
    printf("before freeReplyObject.\n");
	freeReplyObject(reply);
    printf("after freeReplyObject.\n");
    printf("after freeReplyObject.\n");
    printf("after freeReplyObject.\n");
	return retn;
}
...
int rop_set_string(redisContext*conn,const char*key,const char * value)
{
	int retn = 0;
	...
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]set string: %s,key: %serror:%s,%s\n", key,value, reply->str,conn->errstr);	
		retn =  -1;
		goto END;
	}
...
int rop_hash_set(redisContext*conn,const char*key,const char * field,const char*value)
{
	...
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]hset error:key: %s,field:%s,value:%s,error:%s\n", key,field,value,conn->errstr);	
		retn =  -1;
		goto END;
	}
...
int rop_hash_get(redisContext*conn,const char*key,const char * field,char*value)
{
	int retn = 0;
	...
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]hget string: %s,key: %s,field:%s,error:%s\n", key,field,conn->errstr);	
		retn =  -1;
		goto END;
	}

    strncpy((char*)value,reply->str,VALUES_ID_SIZE);
...
int rop_get_string(redisContext*conn,const char*key,const char * value)
{
	int retn = 0;
	...
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]get string: %s,key: %serror:%s\n", key,conn->errstr);	
		retn =  -1;
		goto END;
	}
